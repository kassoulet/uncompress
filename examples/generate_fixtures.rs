...
fn generate_test_zip(path: PathBuf) {
    ...
    zip.start_file("content.txt", options)
        .expect("Failed to add file to ZIP");
    zip.write_all(b"Hello from ZIP! This is test content.")
        .expect("Failed to write to ZIP");

    zip.start_file("data.json", options)
        .expect("Failed to add file to ZIP");
    zip.write_all(b"{\"key\": \"value\", \"number\": 42}")
        .expect("Failed to write to ZIP");

    zip.start_file("subdir/nested.txt", options)
        .expect("Failed to add nested file");
    zip.write_all(b"Nested file content")
        .expect("Failed to write nested content");
...
fn generate_test_gz(path: PathBuf) {
    ...
    encoder
        .write_all(b"This is compressed test data for GZ fixtures.")
        .expect("Failed to write GZ");
